...
function makePrisma(over: Record<string, unknown> = {}) {
  const base = {
    ...
    variation: { upsert: jest.fn().mockResolvedValue({}) },
  };
...
describe('PancakeSyncService.syncProducts', () => {
  it('chưa cấu hình → trả 0, không fetch', async () => {
    ...
    const n = await new PancakeSyncService(makePrisma(), client).syncProducts();
    expect(n).toBe(0);
  ...
  it('upsert hết sản phẩm 1 trang ngắn → dừng', async () => {
    ...
    const n = await new PancakeSyncService(prisma, makeClient([[prod('a'), prod('b')]])).syncProducts();
    expect(n).toBe(2);
  ...
  it('cô lập lỗi từng sản phẩm — 1 SP hỏng không làm hỏng cả batch', async () => {
    ...
    const n = await new PancakeSyncService(prisma, makeClient([[prod('bad'), prod('good')]])).syncProducts();
  ...
  it('SP đã tồn tại → update KHÔNG đụng slug/brand (giữ trường Tubu tự quản)', async () => {
    ...
    await new PancakeSyncService(prisma, makeClient([[prod('a')]])).syncProducts();
...
describe('PancakeSyncService.slugify', () => {
  const slug = (name: string, suffix: string) =>
    (new PancakeSyncService(makePrisma(), makeClient([])) as unknown as {
