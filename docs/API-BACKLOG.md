...
|---|------|-----------|-----------|---------|
| A1 | **Cron refresh Zalo OA Access Token** | Token ZNS hết hạn ~25h → sau đó ZNS **gửi fail** cho tới khi nạp lại token thủ công. Cần lưu `refresh_token` + cron gọi Zalo API làm mới token định kỳ. | Cần OA đã tạo + refresh_token (sau khi đăng ký OA). **Code viết được ngay**, chỉ chờ có OA để chạy. | **CAO** (bắt buộc trước khi dựa vào ZNS) |
