...
| 6.3 Cart & checkout B2C | ✅ | cart+coupon, quote, place-order **idempotent + atomic ví/điểm**, COD/ZaloPay/Ví/XU/**chuyển khoản VietQR** (2026-06-26: sinh QR Napas phía server, Pancake đối soát→webhook lật PAID, màn `/bank-payment`). 🟡 VNPay (đã chọn ZaloPay), isCombo, PENDING_SYNC-retry: hoãn |
