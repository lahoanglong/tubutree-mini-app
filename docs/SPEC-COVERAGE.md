...
|-----|-----------|---------|
| 6.1 Auth & user | ✅ | Zalo Mini App login + web OAuth, JWT access/refresh **rotation atomic + chống reuse**, RBAC guards |
...
| 6.7 Gamification | ✅ | check-in/streak, spin (Điểm Xanh), quiz, tưới cây→thu hoạch, **cây thật + chứng nhận**, missions, leaderboard |
...
## Tổng kết
- **Toàn bộ Phase-1 user-facing đã hoàn thiện** — miniapp dùng được full chức năng (với COD/Ví; ZaloPay/ZNS/Pancake/Accesstrade bật khi có key).
- **194 unit test pass**, 28 suite. 3-app build sạch. API boot + health OK.
